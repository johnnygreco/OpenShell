...
- The initial agent bridge supports exactly one configured agent-conversation binding, one exact provider host, and combined sandbox topology. Adding the first agent binding to a running sandbox requires recreating that sandbox so the loopback listener and environment variable are present.
